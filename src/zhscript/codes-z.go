...
func (this *codes___) z__(code []rune, up_qv *Qv___, from, to, end_mask, mask, lvl int, thiz, thiz1 *buf_codes___) (int, *Keyword___, *Errinfo___) {
	if o_liucheng_ {
		o_n__()
    ...
    for i < to {
    	...
    	if !ok {
    		if yinhaonei == 0 && yuanyangnei == 0 && code_nei == 0 && (end_mask & m_bidanyinhao_ == 0) {
	    		i, kw, thiz2, ok = is_def__(code, up_qv, i, thiz, thiz1)
	    	}
    	}
    	if ok {
			switch {
			...
			default:
				...
				for {
					i2, kw2, err2 = this.z__(code, up_qv, from2, to, end_mask2, mask | end_mask2, lvl + 1, thiz2, thiz)
					if err2 != nil {
						if o_liucheng_ {
					...
					switch kw2 {
					case nil:
					case Kws_.Kaihuakuohao, Kws_.Kaifangkuohao:
					case Kws_.Dunhao:
						thiz2.separ__()
					default:
						switch {
						...
						case kw2 == Kws_.Then || kw == Kws_.Then || kw2 == Kws_.Else || kw == Kws_.Else:
							...
							switch thiz2.kw {
							case Kws_.Then:
								thiz2.to1__(if2.then)
							case Kws_.Else:
								thiz2.to1__(if2.else1)
							default:
								thiz2.to1__(if2.logic)
							}
...
func (this *codes___) z3__(thiz, thiz2 *buf_codes___, if2 *code_logic___, var2 *code_var___,
kw *Keyword___, i, i2 int, code []rune) (*Errinfo___) {
	if o_liucheng_ {
		o__('x', "%s", kw)
	}
	switch kw {
	...
	default:
		...
		switch kw {
		case Kws_.Kaidanyinhao:
			codes = thiz.add_2__(Kws_.Kaidanyinhao, Kws_.Bidanyinhao)
		case Kws_.Kaifangkuohao:
			codes = thiz.add_2__(Kws_.Kaifangkuohao, Kws_.Bifangkuohao)
		case Kws_.Kaihuakuohao:
			codes = thiz.add_2__(Kws_.Kaihuakuohao, Kws_.Bihuakuohao)
		default:
			codes = thiz.add_2__(kw, nil)
		}
		thiz2.to1__(codes)
	}
	return nil
...
func (this *codes___) z2__(code []rune, up_qv *Qv___) *Errinfo___ {
	...
	_, _, err := this.z__(code, up_qv, 0, len(code), 0, m_all_, 0, buf, nil)
	if err == nil {
		buf.to1__(this)
		if o_tree_ {
